...
export default function Query(name, constructor) {
  ...
  function _query(...args) {
    ...
    if (isFunction(gen)) {
      this[Symbol.iterator] = (...iterArgs) =>
        gen.apply(this, iterArgs)[Symbol.iterator](...iterArgs);
    } else if (isObject(gen) && isFunction(gen[Symbol.iterator])) {
      this[Symbol.iterator] = (...iterArgs) =>
        gen[Symbol.iterator](...iterArgs);
    } else {
      throw new Error(
        "Query's constructor function should return a generator or iterator"
      );
    }
  }
